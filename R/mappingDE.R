mappingDE <- function(data, var = NULL, colID = NULL,
  ...
  
{
  ...
  if(!inherits(data, "DE"))
  {
    ...
    matchWith <- match.arg(matchWith, choices = eval(formals(mappingDE)$matchWith))
    
    data <- DE(data = data, colID = colID, unit = unit, matchWith = matchWith, subset = subset,
               check.unit.names = check.unit.names, use_cache = use_cache, use_internet = use_internet, crs = options$crs,
  ...
  }else
  {
    ...
    if(!is.null(aggregation_unit))
    {
      ...
      if(any(aggregation_unit%in%c("state","district", "municipal", "municipality")))
      {
        ...
        nm <- getNamesDE(unit = unit, all_levels = TRUE)
